...
fun HeroWithSeriesAndComics.toHero(): Hero{
    return  Hero(this.heroWithComics.hero.heroId,this.heroWithComics.hero.description,this.heroWithComics.hero.date,this.heroWithComics.hero.heroName,
            this.series?.map { it.toSerie() },this.heroWithComics.comics?.map { it.toComic() })
}
fun HeroEntity.toHero() : Hero = Hero(this.heroId,this.description,this.date,this.heroName,null,null)
...
fun Hero.toHeroWithComics(): HeroWithComics = HeroWithComics(this.toHeroEntity(),this.comics?.map { it.toComicEntity()})

fun Hero.toHeroWithSeriesAndComics(): HeroWithSeriesAndComics = HeroWithSeriesAndComics(this.toHeroWithComics(),this.series?.map { it.toSerieEntity()})
